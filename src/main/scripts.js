...
// Start here, this will call itself when the page loads.
(function start() {
    setTimeout(toBe, 1000); // Start with "To be, "
})();

// Callback function 1
function toBe() {
    opening.textContent += 'To be, ';
    setTimeout(orNotToBe, 1000); // Call "or not to be, "
}

// Callback function 2
function orNotToBe() {
    opening.textContent += 'or not to be, ';
    setTimeout(thatIsTheQuestion, 1000); // Call "that is the question: "
}

// Callback function 3
function thatIsTheQuestion() {
    opening.textContent += 'that is the question: ';
    setTimeout(finishSpeech, 1000); // Finish the rest of the speech
}

// Callback function 4 (final speech part)
function finishSpeech() {
    theRest.innerText += `Whether 'tis nobler in the mind to suffer
    The slings and arrows of outrageous fortune,
    Or to take arms against a sea of troubles
    ...
    And enterprises of great pith and moment
    With this regard their currents turn awry
    And lose the name of action.`; // Hamlet's full speech
}
